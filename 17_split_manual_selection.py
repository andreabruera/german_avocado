...
to_be_split = dict()
with open('split_instructions.tsv') as i:
    for l_i, l in enumerate(i):
        if l_i == 0:
            continue
        line = l.strip().split('\t')
        ...
        word = line[0].strip()
        cat = line[1].strip()
        if cat not in to_be_split.keys():
            to_be_split[cat] = dict()
        separation = line[2].strip()
        try:
            to_be_split[cat][separation].append(word)
        except KeyError:
            to_be_split[cat][separation] = [word]
...
for amount_stim in [
    ...
    for k, v in original_words.items():
        ### considering manually split words
        two_lists['one'][k] = [to_be_split[k][w][0] for w in sorted(to_be_split[k].keys())]
        two_lists['two'][k] = [to_be_split[k][w][1] for w in sorted(to_be_split[k].keys())]
        remaining_words = [w for w in v if w not in two_lists['one'][k] and w not in two_lists['two'][k]]
        assert len(remaining_words) % 2 == 0
        two_lists['one'][k].extend([w for w in remaining_words[:int(len(remaining_words)/2)]])
        two_lists['two'][k].extend([w for w in remaining_words[int(len(remaining_words)/2):]])
